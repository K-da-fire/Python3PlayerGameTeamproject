...
def draw_ui(canvas, canvas_width, UI_HEIGHT,
    ...
    # P1 상태 및 스킬
    canvas.create_text(100, 20, text=f"P1 ♥:{p1_hp} 🔑:{p1_keys}", font=("Arial", 12))
    draw_skill_bar(canvas, 40, 40, p1_selected, p1_skills.skills)

    # P2 상태 및 스킬
    canvas.create_text(300, 20, text=f"P2 ♥:{p2_hp} 🔑:{p2_keys}", font=("Arial", 12))
    draw_skill_bar(canvas, 240, 40, p2_selected, p2_skills.skills)

    # P3 상태 및 스킬
    canvas.create_text(500, 20, text=f"P3 ♥:{p3_hp}", font=("Arial", 12))
    draw_skill_bar(canvas, 430, 40, p3_selected, p3_skills.skills)

    # 남은 시간
    canvas.create_text(700, 20, text=f"⏱ {int(time_left)}초", font=("Arial", 12))
...
def draw_skill_bar(canvas, x, y, selected_index, skills):
    ...
    for i, skill in enumerate(skills):
        ...
        canvas.create_rectangle(bar_x, bar_y, bar_x + bar_width, bar_y + bar_height, fill=fill_color)

        # 쿨타임 오버레이
        cooldown_width = int(bar_width * cooldown_ratio)
        if cooldown_width > 0:
            canvas.create_rectangle(bar_x, bar_y, bar_x + cooldown_width, bar_y + bar_height, fill="gray")

        # 남은 사용 횟수 표시
        canvas.create_text(bar_x + bar_width // 2, bar_y + bar_height + 10,
                           text=f"{skill.remaining_count}", font=("Arial", 10))